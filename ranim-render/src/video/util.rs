...
/// Converts RGBA to YUV data.
///
/// Very much the same as [`openh264::formats::RBGYUVConverter`], but designed for RGBA data
/// that `wgpu` can natively output to; the alpha channel is entirely ignored.
///
/// This is really just a lazy workaround to not waste more memory to convert the RGBA data
/// to RGB.
pub struct RgbaYuvConverter {
    yuv: Vec<u8>,
    width: usize,
    height: usize,
}

impl RgbaYuvConverter {
    ...
    /// Converts the RGBA array. The alpha component is completely ignored.
    pub fn convert(&mut self, rgba: &[u8]) {
        ...
        for i in 0..width / 2 {
            for j in 0..height / 2 {
                let px = i * 2;
                let py = j * 2;
                let pix0x0 = sample(px, py);
                let pix0x1 = sample(px, py + 1);
                let pix1x0 = sample(px + 1, py);
                let pix1x1 = sample(px + 1, py + 1);
                let avg_pix = (
...
impl YUVSource for RgbaYuvConverter {
    fn width(&self) -> i32 {
        self.width as i32
    }
...
#[cfg(test)]
mod tests {
    use super::RgbaYuvConverter;
    use openh264::formats::YUVSource;

    #[test]
    fn rgb_to_yuv_conversion_black_2x2() {
        let mut converter = RgbaYuvConverter::new(2, 2);
        let rgb = [0u8, 0, 0, 255, 0, 0, 0, 255, 0, 0, 0, 255, 0, 0, 0, 255];
    ...
    fn rgb_to_yuv_conversion_white_4x2() {
        let mut converter = RgbaYuvConverter::new(4, 2);
        let rgb = [
    ...
    fn rgb_to_yuv_conversion_red_2x4() {
        let mut converter = RgbaYuvConverter::new(4, 2);
        let rgb = [
            255u8, 0u8, 0u8, 255u8, 255u8, 0u8, 0u8, 255u8, 255u8, 0u8, 0u8, 255u8, 255u8, 0u8,
            0u8, 255u8, 255u8, 0u8, 0u8, 255u8, 255u8, 0u8, 0u8, 255u8, 255u8, 0u8, 0u8, 255u8,
            255u8, 0u8, 0u8, 255u8,
        ];
        converter.convert(&rgb);
        assert_eq!(
